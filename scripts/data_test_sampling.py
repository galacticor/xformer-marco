...
for i in range(len(conditions)):
    ...
    qid_exclude = list(top100[ 
        (top100.label == 1) & (top100.is_condition == False)
    ].qid.unique())
    ...
    tmpdf = pd.DataFrame({
        'total' : top100val.groupby(["qid", "is_condition"]).size()
    }).reset_index()
    tmpdf = tmpdf[
        (tmpdf.is_condition == True) & (~tmpdf.qid.isin(qid_exclude))
    ].sort_values(["total"], ascending=False)[:100]
